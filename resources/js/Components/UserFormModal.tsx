...
export default function UserFormModal({ isOpen, onClose, type, user }: Props) {
  ...
  // Reset & isi form otomatis saat modal dibuka/ditutup
  useEffect(() => {
    if (isOpen) {
      clearErrors();
      if (type === 'edit' && user) {
        setData({
          name: user.name,
          email: user.email,
          password: '', // Kosongkan agar tidak perlu diisi ulang kecuali mau diubah
          role: user.roles[0]?.name || 'petugas',
          phone: user.phone || '',
          nik: user.nik || '',
        });
      } else {
        reset(); // Reset ke awal jika mode "Tambah"
      }
    }
  }, [isOpen, type, user]);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full border border-slate-300 rounded-xl p-2.5 text-sm outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500"
          ...
          {/* Hanya tampilkan opsi Role saat membuat user baru */}
          {type === 'add' && (
            <div>
              ...
              <select
                ...
                className="w-full border border-slate-300 rounded-xl p-2.5 text-sm outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 bg-white"
              >
          ...
          <div className="flex justify-end gap-2 pt-4 border-t border-slate-100 mt-6">
            ...
            <button
              type="submit"
              disabled={processing}
              className="px-4 py-2 text-sm bg-orange-600 text-white font-semibold rounded-xl hover:bg-orange-700 disabled:opacity-50 transition-colors"
            >
